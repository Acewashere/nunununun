...
client.once('ready', () => {
  log.info(` successfully logged in as ${client.user.tag}`); // Say hello to console
  client.user.setActivity(`${config.prefix}help is the help command selling src east#1337 ${client.user.username.toLowerCase()}`, { type: "WATCHING" }); // Set the bot's activity status
  ...
  if (config.command.notfound_message === true && !client.commands.has(command)) {
    return message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setTitle('Unknown command faggot :(')
        .setDescription(`nigga you dumb fuck i couldn't find the  \`${command}\` command`)
